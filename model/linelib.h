...
public:
    ...
    friend class line_trainer_feature;
...
protected:
    ...
    void train_uv_od3_attention(int u, int v, real lr, int neg_samples, real *_error_vec, unsigned long long &rand_index, real lr2, real *_para);
public:
    line_trainer_line();
    ~line_trainer_line();
    ...
    void train_sample_od3_attention(real lr, int neg_samples, real *_error_vec, double (*func_rand_num)(), unsigned long long &rand_index, real lr2, real *_para);
...
protected:
    ...
    line_node *lab, *fea;
    std::vector<int> data_lab;
    std::vector< std::vector<int> > data_fea;
    int data_size;
    
    void ReadWord(char *word, FILE *fin);
    void Softmax(BLPMatrix &mat);
public:
    line_trainer_feature();
    ~line_trainer_feature();
    
    void init(const char *file_name, line_node *p_label, line_node *p_feature);
    void train_sample(real lr, double (*func_rand_num)());
};
